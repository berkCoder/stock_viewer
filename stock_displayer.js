...
(function () {
  ...
  // Map each UI range to the candle interval that looks best for it.
  ...
  function verdict(a, data) {
    // Score several signals into a single bull/bear/flat read.
    let score = 0;
    ...
    if (a.drawFromHigh > -3) score += 1;        // near its highs
    if (a.upFromLow < 3) score -= 1;            // sitting near its lows
  ...
  function renderChart(data, a) {
    ...
    // Time unit that keeps the x-axis readable per range.
    const unit =
    ...
    // Press and drag across the chart to measure the price move between two
    // points (Google-Finance style): a shaded band, dashed markers at each
    // end, dots on the price line, and a label with the absolute / percent
    // difference and the time span. A plain click (no drag) does nothing
    // special, leaving the normal hover tooltip intact.
    ...
    const dragCompare = {
      ...
      afterDestroy(c) {
        ...
        const canvas = state._canvas; // c.canvas is null by destroy time
        if (canvas) {
      ...
      // Hide the default hover tooltip while a drag selection is active.
      beforeTooltipDraw(c) {
      ...
      afterDraw(c) {
        ...
        if (s.anchor === s.current) return; // need two distinct points
        ...
        // Dots on the price line at each end.
        ...
        // Label: signed price move, percentage, and the time span.
        const arrow = up ? "▲" : "▼";
    ...
    chart = new Chart(ctx, {
      ...
      data: {
        datasets: [
          ...
          {
            // Average reference line — highlights the trajectory vs mean.
            label: "Average",
  ...
  // --- boot -----------------------------------------------------------
  // Show a well-known symbol on first load so the page has real data to
  // render immediately.
  input.value = "AAPL";
  load("AAPL");
})();
